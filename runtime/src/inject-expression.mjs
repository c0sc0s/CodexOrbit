...
export const RUNTIME_VERSION = "3.3.0";
...
function installRuntime(config) {
  ...
  const state = { query: "", tag: "all", sort: "sidebar", sortOpen: false, open: false, view: "sessions", tagError: "" };
  ...
  const titleNodes = () => Array.from(document.querySelectorAll("[data-thread-title]"))
    .filter((node) => node instanceof HTMLElement && !node.closest(`#${TOOLBAR_ID}`));
  ...
  const ensureToolbar = (nodes) => {
    ...
    if (host) return host;
    const pluginsButton = Array.from(document.querySelectorAll("button"))
    ...
    const pinnedToggle = pinnedToggleRef?.isConnected
      ? pinnedToggleRef
      : document.querySelector("[data-app-action-sidebar-section-toggle]");
  ...
  const projectIdForRow = (row) => {
    for (let parent = row?.parentElement; parent; parent = parent.parentElement) {
      const projectRows = parent.querySelectorAll("[data-app-action-sidebar-project-row]");
  ...
  const sectionToggleForRow = (row) => {
    for (let parent = row?.parentElement; parent; parent = parent.parentElement) {
      const toggles = parent.querySelectorAll("[data-app-action-sidebar-section-toggle]");
  ...
  const entriesFrom = (nodes) => {
    nodes.forEach((node, index) => {
      ...
      const parsed = parse(raw);
      if (!parsed) return;
      const row = node.closest("[data-app-action-sidebar-thread-row]");
  ...
  const findThreadRow = (threadId) => {
    ...
    return Array.from(document.querySelectorAll("[data-app-action-sidebar-thread-row]"))
      .find((row) => row.getAttribute("data-app-action-sidebar-thread-id") === threadId && row.getClientRects().length > 0) ?? null;
  };

  const openEntry = async (entry) => {
    let row = entry.row?.isConnected && entry.row.getClientRects().length > 0 ? entry.row : findThreadRow(entry.threadId);
    ...
    if (!row && entry.projectId) {
      const projectRow = Array.from(document.querySelectorAll("[data-app-action-sidebar-project-row]"))
        .find((item) => item.getAttribute("data-app-action-sidebar-project-id") === entry.projectId);
    ...
    for (let attempt = 0; !row && attempt < 80; attempt += 1) {
      ...
      row = findThreadRow(entry.threadId);
    }
    if (row) {
      state.query = "";
      state.tag = "all";
  ...
  const visibleEntries = (entries) => {
    ...
    if (state.sort === "time") return filtered.sort((a, b) => timeRank(b.time) - timeRank(a.time) || a.index - b.index);
    if (state.sort === "tag") return filtered.sort((a, b) => a.tag.localeCompare(b.tag, "zh-CN") || a.title.localeCompare(b.title, "zh-CN"));
    if (state.sort === "title") return filtered.sort((a, b) => a.title.localeCompare(b.title, "zh-CN", { numeric: true }));
  ...
  const appendHighlightedText = (element, text, query) => {
    ...
    for (const match of text.matchAll(matcher)) {
      ...
      mark.className = "codex-sidebar-search-mark";
      mark.textContent = match[0];
      element.append(mark);
      cursor = index + match[0].length;
    }
    if (cursor === 0) element.textContent = text;
    else if (cursor < text.length) element.append(document.createTextNode(text.slice(cursor)));
  };
  ...
  const renderToolbar = (entries, reason = "state") => {
    ...
    if (state.view === "sessions") {
      ...
      const results = visibleEntries(entries);
      ...
      results.forEach((entry) => {
        if (state.sort === "tag" && entry.tag !== lastGroup) {
          lastGroup = entry.tag;
          const heading = document.createElement("div");
          heading.className = "codex-sidebar-result-group";
          heading.textContent = entry.tag;
          list.appendChild(heading);
        }
        const result = button("codex-sidebar-result", "");
        result.setAttribute("role", "listitem");
        result.title = entry.raw;
        const tag = document.createElement("span");
        tag.className = "codex-sidebar-result-tag";
        tag.dataset.tone = entry.tone;
        tag.textContent = entry.tag;
        const resultContent = document.createElement("span");
        resultContent.className = "codex-sidebar-result-content";
        const title = document.createElement("span");
        title.className = "codex-sidebar-result-title";
        appendHighlightedText(title, entry.title, state.query);
        resultContent.appendChild(title);
        if (entry.snippet) {
          const snippet = document.createElement("span");
          snippet.className = "codex-sidebar-result-snippet";
          appendHighlightedText(snippet, entry.snippet, state.query);
          resultContent.appendChild(snippet);
        }
        result.append(tag, resultContent);
        result.addEventListener("click", () => { void openEntry(entry); });
        list.appendChild(result);
      });
      if (results.length === 0) {
        const empty = document.createElement("div");
        empty.className = "codex-sidebar-results-empty";
  ...
  const containsSidebarNode = (node) => node instanceof Element
    && (node.matches(sidebarSelector) || Boolean(node.querySelector(sidebarSelector)));
  const isRelevantMutation = (mutation) => {
    const target = mutation.target instanceof Element ? mutation.target : mutation.target.parentElement;
    if (!target || host?.contains(target) || modal?.contains(target)) return false;
    if (target.closest("[data-thread-title]")) return true;
    return [...mutation.addedNodes, ...mutation.removedNodes].some(containsSidebarNode);
  };
...
export function buildInjectionExpression() {
  ...
  return `(${installRuntime.toString()})(${JSON.stringify(config)})`;
}
