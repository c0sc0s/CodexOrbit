...
export async function readSidebarMembership(root) {
  ...
  for (const [project, order] of Object.entries(state["sidebar-project-thread-orders"])) {
    ...
    for (const id of ids(order?.threadIds)) assignments.set(id, project);
  }
  ...
  for (const [id, assignment] of Object.entries(state["thread-project-assignments"])) {
    if (record(assignment) && assignment.projectKind === "local" && projects.has(assignment.projectId)) {
      assignments.set(id.replace(/^local:/u, ""), assignment.projectId);
    } else assignments.delete(id.replace(/^local:/u, ""));
  }
  return {
    forThread(row) {
      const projectId = assignments.get(row.threadId) ?? (projects.has(row.projectId) ? row.projectId : null);
