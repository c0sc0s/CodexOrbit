...
export function installLoader(createResourceScope, version, createRendererTransport) {
  const existing = window.__codexPluginLoader;
  if (existing) {
    if (existing.apiVersion === 1 && existing.version === version) return existing;
    if (existing.apiVersion !== 1 || existing.status().length) throw new Error("Incompatible Codex Plugin Loader is active; unload its plugins before upgrading");
  }
  ...
  const loader = {
    ...
    status: (id) => id === undefined ? [...plugins.values()].map(summary) : summary(plugins.get(id)),
    load: (manifest, activate, config = {}) => {
      ...
      return serialize(manifest.id, async () => {
        ...
        const record = { id: manifest.id, version: manifest.version, instanceId: manifest.instanceId, state: "loading", scope: createResourceScope(), instance: null };
        ...
        try {
          ...
          await clean(record);
          record.state = error?.message === "Activation timed out" ? "cleanup-failed" : "failed";
