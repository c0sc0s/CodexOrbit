...
export async function createLauncher({ launcherPath, nodePath, cliPath, configPath, port = 9341, logPath, iconPath, bundleId = "io.github.c0sc0s.codex-plugin-loader" }) {
  try {
  ...
  try {
    ...
    const script = `#!/bin/sh\nif ! ${[nodePath, cliPath, "start", "--config", configPath, "--port", String(port)].map(quoteShell).join(" ")} >> ${quoteShell(logPath)} 2>&1 </dev/null; then\n  /usr/bin/osascript -e 'display dialog "Loader could not start. If Codex is open without debugging, quit it completely and reopen this launcher. Otherwise check the Loader log." with title "Codex Plugin Loader" buttons {"OK"} default button "OK"'\n  exit 1\nfi\n`;
