...
import { createServiceModule } from "../service/module.js";
import { createResourceScope } from "../lifecycle/resource-scope.js";
import { readManifest } from "../config/manifest.js";

import type { PluginTargetRegistry } from "../cdp/target-registry.js";
import type { CdpClient } from "../cdp/client.js";
import type { PluginEntry } from "../sdk/contracts.js";
import type { LoaderManifest, PluginResult, CleanupFailure } from "../protocol/types.js";
import type { ResourceScope } from "../lifecycle/resource-scope.js";
import type { HostModule } from "../service/module.js";
interface HostClient { client: CdpClient; generation: number; ready: boolean; bound: boolean; bindings: string[] }
interface ModuleRecord { id: string; scope: ResourceScope; module: HostModule | null; clients: Map<string, HostClient>; pending: Set<Promise<unknown>>; error: string | null; signature: string; retiring?: boolean }
/** Runs configured host modules and their renderer plugins without product-specific branches. */
export class PluginHost {
  registry: PluginTargetRegistry;
  manifest: LoaderManifest;
  createModule: typeof createServiceModule;
  modules: ModuleRecord[];
  syncing: Promise<PluginResult[]> | null;
  closing: Promise<CleanupFailure[]> | null;
  constructor({ registry, manifest, createModule = createServiceModule }: { registry: PluginTargetRegistry; manifest: LoaderManifest; createModule?: typeof createServiceModule }) {
  ...
  async add(entry: PluginEntry) {
    const scope = createResourceScope();
    const record: ModuleRecord = { id: entry.id, scope, module: null, clients: new Map(), pending: new Set(), error: null, signature: JSON.stringify(entry) };
  ...
  async synchronize() {
    ...
    for (const record of this.modules) {
      if (!record.module || record.scope.signal.aborted) { results.push({ pluginId: record.id, stage: record.error ?? undefined, error: "Host module unavailable" }); continue; }
        ...
        for (const target of targets) {
          ...
          try {
            ...
            if (changed) {
              ...
              const state: HostClient = { client, generation: client.generation, ready: false, bound: false, bindings: [] };
            ...
            if (error instanceof Error && (error.message === "Module ready timed out" || error.message === "Target cleanup timed out")) { record.scope.abort(); record.error = stage; }
  ...
  async removeRecord(record: ModuleRecord) {
    record.scope.abort();
