...
import type { PluginResult } from "../protocol/types.js";
export type DaemonPaths = ReturnType<typeof daemonPaths>;
interface OwnerRecord { pid: number; token: string; phase: string; scriptPath: string; command: string }
interface HostStatus { id: string; state: string }
const run = promisify(execFile);
const cliPath = fileURLToPath(new URL("../cli.js", import.meta.url));

export function daemonPaths(configPath: string, port: number) {
...
export async function readOwner(paths: DaemonPaths): Promise<OwnerRecord | null> {
  try {
...
export async function daemonStatus(paths: DaemonPaths, expectedCli = cliPath) {
  ...
  const owned = record.scriptPath === expectedCli && typeof record.command === "string"
    && record.command.startsWith(`${expectedCli} `) && stdout.trim().endsWith(record.command);
...
export async function acquireOwner(paths: DaemonPaths) {
...
export async function claimOwner(paths: DaemonPaths, token: string, phase = "starting") {
...
export async function releaseOwner(paths: DaemonPaths, token: string) {
  if ((await readOwner(paths))?.token === token) await rm(paths.lease, { force: true });
}

export async function startDaemon({ configPath, port, paths }: { configPath: string; port: number; paths: DaemonPaths }) {
  ...
  try {
    ...
    const result = await new Promise<PluginResult[]>((resolveReady, reject) => {
      const timer = setTimeout(() => done(new Error("Loader startup timed out; see loader.log")), 30_000);
      const done = (error: Error | null, value: PluginResult[] = []) => {
        clearTimeout(timer);
        launched.removeListener("error", onError);
        launched.removeListener("exit", onExit);
        launched.removeListener("message", onMessage);
      ...
      const onMessage = (value: unknown) => { const message = value as { type?: string; result: PluginResult[] }; if (message?.type === "ready") done(null, message.result); };
      launched.once("error", onError);
      launched.once("exit", onExit);
      launched.on("message", onMessage);
    });
    child.disconnect();
    child.unref();
    ...
    if (child?.pid) {
      ...
      if (child.exitCode === null && child.signalCode === null) await new Promise<void>((resolveExit) => {
...
export async function stopDaemon(paths: DaemonPaths) {
  const status = await daemonStatus(paths);
...
export async function writeHostStatus(paths: DaemonPaths, token: string, modules: HostStatus[]) {
...
export async function readHostStatus(paths: DaemonPaths): Promise<HostStatus[] | null> {
