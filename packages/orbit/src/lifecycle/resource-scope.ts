...
export function createResourceScope(timeoutMs = 5000) {
  ...
  const disposers: Array<() => MaybePromise<unknown>> = [];
  let closing: Promise<void> | null = null;
  const bounded = <T>(operation: () => MaybePromise<T>, label: string): Promise<T> => new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error(`${label} timed out`)), timeoutMs);
    Promise.resolve().then(operation).then(resolve, reject).finally(() => clearTimeout(timer));
  });
  const onDispose = (dispose: () => MaybePromise<unknown>) => {
