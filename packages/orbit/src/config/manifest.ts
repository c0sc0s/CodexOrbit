...
import type { LoaderManifest } from "../protocol/types.js";

/** Reads identities without touching executable entries, so status/removal survives a missing bundle. */
export async function readManifest(configPath: string): Promise<LoaderManifest> {
...
export async function resolvePluginEntry(root: string, entry: string) {
...
export async function readPlugins(configPath: string, owner: string | null = null) {
...
export async function setPluginEnabled(configPath: string, id: string, enabled: boolean) {
  const path = await realpath(configPath);
  ...
  try {
    ...
    if (config.orbit) throw new Error("Use Orbit plugin commands to change managed platform configuration");
    const plugin = config.plugins.find((entry: PluginEntry) => entry.id === id);
