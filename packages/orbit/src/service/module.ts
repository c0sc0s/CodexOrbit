...
import { buildPluginExpression } from "../cdp/expressions.js";
import { resolvePluginEntry } from "../config/manifest.js";
import { ServiceProcess } from "./process.js";

import type { CdpClient } from "../cdp/client.js";
import type { PluginEntry, MaybePromise } from "../sdk/contracts.js";
import type { PluginDescriptor, PluginSummary, BindingCall } from "../protocol/types.js";
...
export interface HostModule {
  renderer(client: CdpClient): MaybePromise<PluginDescriptor>;
  bindings?: Array<{ name: string; handle(client: CdpClient, params: BindingCall): MaybePromise<unknown> }>;
  cancelPending?(): void;
  tick?(): MaybePromise<void>;
  onReady?(client: CdpClient): MaybePromise<unknown>;
  onTargetRemoved?(id: string): MaybePromise<void>;
  dispose?(): MaybePromise<void>;
}
interface ClientState { client: CdpClient; token: string; generation: number; contextId?: number; pending: Map<number, AbortController>; descriptor?: PluginDescriptor }
/** Adapts the business SDK to the host's private target lifecycle. */
export async function createServiceModule(entry: PluginEntry, { root, stateDirectory, owner, onDispose }: { root: string; stateDirectory: string; owner: string | null; onDispose: ResourceScope["onDispose"] }): Promise<HostModule> {
  ...
  const deliver = async (state: ClientState, message: ServiceEvent | ServiceReply) => {
  ...
  return {
    ...
    async renderer(client) {
      ...
      const status = await client.evaluate<PluginSummary | null>(`window.__codexPluginLoader?.status(${JSON.stringify(entry.id)})`);
