...
import type { ServiceContext, MaybePromise } from "../sdk/contracts.js";
import type { ServiceRequest, ServiceReply, ServiceEvent } from "../protocol/ipc.js";
type ClientCallback = (clientId: string) => MaybePromise<unknown>;
type RpcHandler = Parameters<ServiceContext["rpc"]["handle"]>[1];
const scope = createResourceScope(1000);
const handlers = new Map<string, RpcHandler>();
const requests = new Map<number, { controller: AbortController; clientId: string }>();
const clients = new Set<string>();
const connectHandlers = new Set<ClientCallback>();
const disconnectHandlers = new Set<ClientCallback>();
let activated = false;
const send = (message: ServiceEvent | ServiceReply) => {
  assertMessageSize(message);
  if (process.connected) process.send?.(message, () => {});
};
const subscribe = (set: Set<ClientCallback>, callback: ClientCallback) => {
...
async function handle(message: ServiceRequest) {
  const { type } = message;
  ...
  if (type === "activate") {
    ...
    const instance = await imported.activate(Object.freeze<ServiceContext>({
  ...
  requests.set(message.id, { controller, clientId: message.clientId });
...
process.on("message", (value: unknown) => {
  ...
  const message = value as ServiceRequest;
  Promise.resolve().then(() => handle(message)).then(
    ...
    () => { if (message.type !== "cancel") send({ id: message.id, error: "Plugin service request failed" }); },
  ).catch(() => {});
});
