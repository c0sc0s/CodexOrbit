...
type ProcessOptions = { port?: number; appPath?: string; executable?: string; run?: (file: string, args: string[]) => Promise<{ stdout: string; stderr: string }>; spawn?: typeof spawn };
...
export function findCodexApp() {
  for (const appPath of ["/Applications/Codex.app", join(homedir(), "Applications", "Codex.app"), "/Applications/ChatGPT.app"]) {
    try {
      ...
      const read = (key: string) => execFileSync("/usr/bin/plutil", ["-extract", key, "raw", plist], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: 2000 }).trim();
...
export class CodexProcess {
  ...
  appPath: string;
  executable: string;
  run: NonNullable<ProcessOptions["run"]>;
  spawn: typeof spawn;
  ...
  async cdpIsReady(discoverTargets: () => Promise<Target[]>) {
    try {
  ...
  async waitForCdp(discoverTargets: () => Promise<Target[]>, timeoutMs = 30_000) {
  ...
  async launchWithCdp(discoverTargets: () => Promise<Target[]>) {
    if (await this.isRunning()) throw new Error("Codex is already running without debugging. Quit Codex completely, then use the explicit loader entry again. The running app was not restarted.");
