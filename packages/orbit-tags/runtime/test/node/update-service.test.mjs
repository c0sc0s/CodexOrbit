...
import { UpdateService, isNewer, isRelease } from "../../../dist/runtime/src/service/update/service.js";
import { runUpdate, writeUpdateStatus } from "../../../dist/runtime/src/service/update/worker.js";

async function fixture(t) {
  const root = await mkdtemp(join(tmpdir(), "tags-update-test-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  await writeFile(join(root, "install.json"), JSON.stringify({ packageVersion: "0.6.1", platformRoot: root }));
  return root;
}
...
test("worker pins npm package and persists verified completion outside temporary files", async (t) => {
  ...
  await runUpdate({ directory: root, temporary, version: "0.7.0" }, async (executable, args, options) => {
    commands.push(args); assert.equal(options.env.CODEX_TAGS_INSTALL_DIR, root);
    if (args[1] === "plugin" && args[2] === "install") await writeFile(join(root, "install.json"), JSON.stringify({ packageVersion: "0.7.0" }));
  });
  assert.equal(commands[0].at(-1), "@c0sc0s/orbit-tags@0.7.0");
  assert.equal(JSON.parse(await readFile(join(root, "update-status.json"))).phase, "succeeded");
  await assert.rejects(access(temporary));
});
test("worker reports installation failure without bypassing platform recovery", async (t) => {
  const root = await fixture(t); const temporary = await mkdtemp(join(root, "worker-")); let calls = 0;
  await runUpdate({ directory: root, temporary, version: "0.7.0" }, async () => { calls++; throw new Error("failed"); });
  assert.equal(calls, 1); assert.equal(JSON.parse(await readFile(join(root, "update-status.json"))).error, "install");
});
test("detached worker survives service replacement and reports completion to the new service", async (t) => {
  const root = await fixture(t);
  await writeFile(join(root, "orbit.mjs"), `import {writeFileSync} from 'node:fs'; import {join} from 'node:path';
if (process.argv[2] !== 'plugin' || process.argv[3] !== 'install') throw new Error('Expected platform registration');
writeFileSync(join(process.env.CODEX_TAGS_INSTALL_DIR, 'install.json'), JSON.stringify({packageVersion:'0.7.0', platformRoot:process.env.ORBIT_HOME}));`);
