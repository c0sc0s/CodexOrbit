import { DEFAULT_TAG_DEFINITIONS, LEGACY_TONE_COLORS } from "./tag-settings.js";
...
export function parseTitleMetadata(value: unknown) {
...
export function parseSidebarTitle(value: unknown) {
  const parsed = parseTitleMetadata(value);
  return parsed ? { ...parsed, color: colorForTag(parsed.tag) } : null;
}

export type ParsedTitleMetadata = NonNullable<ReturnType<typeof parseTitleMetadata>>;
