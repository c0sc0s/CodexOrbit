...
import { readSidebarMembership } from "./sidebar-membership.js";

interface CatalogRow { threadId: string; raw: string; updatedAt: number; projectId: string | null; pinned: number | null; source: string | null; threadSource: string | null }
// Keep the private Codex database schema isolated and always open it read-only.
export class SessionCatalog {
  constructor(readonly root = process.env.CODEX_HOME ?? join(homedir(), ".codex")) {
  }
  ...
  async read() {
    ...
    try {
      ...
      const columns = new Set((database.pragma("table_info(threads)") as Array<{ name: string }>).map(({ name }) => name));
      ...
      const rows = database.prepare<[], CatalogRow>(`SELECT id AS threadId, ${title} AS raw, ${time} AS updatedAt, ${project} AS projectId, ${pinned} AS pinned, ${source} AS source, ${threadSource} AS threadSource FROM threads WHERE archived = 0 ORDER BY ${time} DESC, id`).all();
