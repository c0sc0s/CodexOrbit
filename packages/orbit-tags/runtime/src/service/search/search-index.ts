...
import { discoverSessionFiles, extractConversationText } from "./content-index.js";

export interface IndexStatus { phase: string; completed?: number; total?: number; changed?: number; message?: string }
interface IndexedRow { thread_id: string; file_path: string; file_size: number; modified_at: number }
interface SearchRow { thread_id: string; role: string; content: string; rank: number }
type SearchOptions = { query: unknown; threadIds: unknown; limit?: unknown };
const defaultResultLimit = 50;
const maximumResultLimit = 100;
const extractionVersion = 1;

function localThreadIdFor(threadId: string) {
  return threadId.includes(":") ? threadId.slice(threadId.lastIndexOf(":") + 1) : threadId;
}

function quotedFtsQuery(query: string) {
...
export class SessionSearchIndex {
  discoverFiles: () => Promise<Map<string, string>>;
  readOnly: boolean;
  database: Database.Database;
  selectIndexedSessions: Database.Statement<[], IndexedRow>;
  deleteMessages!: Database.Statement<[string]>;
  deleteSession!: Database.Statement<[string]>;
  insertMessage!: Database.Statement<[string, string, string]>;
  upsertSession!: Database.Statement<[string, string, number, number]>;
  searchFts!: Database.Statement<[string, string, number], SearchRow>;
  searchShortQuery!: Database.Statement<[string, string, number], SearchRow>;
  constructor(databasePath: string, options: { readOnly?: boolean; discoverFiles?: () => Promise<Map<string, string>> } = {}) {
    ...
    if (!this.readOnly) {
      ...
      if (Number(this.database.pragma("user_version", { simple: true })) < extractionVersion) {
  ...
  async refresh(onProgress: (status: IndexStatus) => void = () => {}) {
  ...
  search({ query, threadIds, limit = defaultResultLimit }: SearchOptions) {
    ...
    const originalIdByLocalId = new Map<string, string>();
  ...
  status() {
    const row = this.database.prepare<[], { count: number }>("SELECT count(*) AS count FROM indexed_sessions").get();
