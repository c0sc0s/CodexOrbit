...
import { SettingsRepository } from "../runtime/src/service/settings/repository.js";

function sessionMarkerPath(dataDirectory: string, sessionId: string) {
...
export function buildNamingContext(tags: Array<{ name: string; description: string }>) {
  const tagList = tags.length > 0
    ? tags.map(({ name, description }) => `- [${name}]${description ? `: ${description}` : ""}`).join("\n")
    : "- [Uncategorized]";
  return [
    "Orbit Tags session naming policy:",
...
async function installedDataDirectory() {
  try {
    const metadata = JSON.parse(await readFile(new URL("../../orbit-installation.json", import.meta.url), "utf8"));
    if (typeof metadata.dataDirectory === "string") return metadata.dataDirectory;
  } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  return join(homedir(), "Library", "Application Support", "Codex Sidebar Tags");
}

export async function readNamingContext(options: { settingsPath?: string } = {}) {
  const settingsPath = options.settingsPath
    ?? process.env.CODEX_TAGS_SETTINGS_PATH
    ?? join(await installedDataDirectory(), "settings.json");
...
export async function handleHook(input: { session_id?: string; hook_event_name?: string; source?: string } | null, options: { dataDirectory?: string; settingsPath?: string } = {}) {
