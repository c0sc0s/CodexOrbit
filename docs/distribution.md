...
No `npm install` or build step is required for a released revision because the browser bundle is committed.
...
5. Injects the versioned UI and indexes user/assistant text from local Codex sessions.
...
7. Package without `node_modules`, `.git`, logs, PID files, screenshots generated by QA, or machine-local configuration.
8. Test install, update, restore, and uninstall from a clean macOS user account.
9. Publish the Git tag/release and, when available, the public marketplace entry.
